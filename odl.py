...
############################# OLD ########################################################
    ...
    def in_port_set_queue_and_output_normal(self, node_id, table_id, priority, in_port, queue_id):
        id = f'table{table_id}_inport_{in_port}_queue_{queue_id}'
        flow = Flow(node_id, table_id, id, priority, self.ns)
        flow.match([Flow.in_port(in_port)])
        flow.instructions([
            ['apply-actions', [
                {'action': [Flow.set_queue(queue_id)], 'order': 0, 'ns': 'f'},
...
class Flow():
    ...
    def set_queue(queue_id):
        ip_set = ['set-queue-action', 'queue-id', queue_id]
        return ip_set
...
if __name__ == '__main__':
    ...
    for port in [1, 2]:
        result = odl.in_port_set_queue_and_output_normal(node_id, table_id, priority, port, q)
