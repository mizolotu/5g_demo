"""The CarliniWagnerL2 attack_carrier.
"""
import numpy as np
...
def carlini_wagner_l2(
    ...
    """
    This is the function interface for the Carlini-Wagner-L2 attack_carrier.
    For more details on the attack_carrier and the parameters see the corresponding class.
    """
    return CarliniWagnerL2(
        model_fn, y, targeted, batch_size, clip_min, clip_max, binary_search_steps, max_iterations, abort_early, confidence, initial_const, learning_rate
    ).attack(x)
...
class CarliniWagnerL2(object):
    def __init__(
        ...
        """
        This attack_carrier was originally proposed by Carlini and Wagner. It is an
        iterative attack_carrier that finds adversarial examples on many defenses that
        are robust to other attacks.
        Paper link: https://arxiv.org/abs/1608.04644
        At a high level, this attack_carrier is an iterative attack_carrier using Adam and
        a specially-chosen loss function to find adversarial examples with
        lower distortion than other attacks. This comes at the cost of speed,
        as this attack_carrier is often much slower than others.

        :param model_fn: a callable that takes an input tensor and returns the model logits.
        :param y: (optional) Tensor with target labels.
        :param targeted: (optional) Targeted attack_carrier?
        ...
        :param learning_rate (optional): The learning rate for the attack_carrier algorithm.
        ...
        self.y = y
        self.targeted = targeted # y is not None
    ...
    def attack(self, x):
        ...
        for i in range(0, len(x), self.batch_size):
            tmp = self._attack(x[i: i + self.batch_size]).numpy()
            adv_ex[i : i + self.batch_size] = tmp
    ...
    def _attack(self, x):
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            for iteration in range(self.max_iterations):
                ...
                mask = tf.reshape(mask, [-1] + [1 for _ in best_attack.shape[1:]])

                mask = tf.tile(mask, [1, *best_attack.shape[1:]])

                #if len(best_attack.shape) < 4:
                #    mask = tf.tile(mask, [1, *best_attack.shape[0:]])
                #else:
                #    mask = tf.tile(mask, [1, *best_attack.shape[1:]])
    ...
    def gradient(self, x, y, modifier, const):
        # compute the actual attack_carrier
...
def loss_fn(
    ...
    #print(y_true.shape, y_pred.shape, real.shape, other.shape)
