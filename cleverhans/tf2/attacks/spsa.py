...
def spsa(
    ...
    :param targeted: (optional) bool. Is the attack_carrier targeted or untargeted? Untargeted, the default,
              will try to make the label incorrect. Targeted will instead try to move in the direction
              of being more like y.
    :param early_stop_loss_threshold: A float or None. If specified, the attack_carrier will end as soon as
...
def projected_optimization(
    ...
    Args:
      ...
      :param early_stop_loss_threshold: A float or None. If specified, the attack_carrier will end if the loss is below
         `early_stop_loss_threshold`.
          Enabling this option can have several different effects:
            - Setting the threshold to 0. guarantees that if a successful attack_carrier is found, it is returned.
              This increases the attack_carrier success rate, because without early stopping the optimizer can accidentally
              bounce back to a point where the attack_carrier fails.
            - Early stopping can make the attack_carrier run faster because it may run for fewer steps.
            - Early stopping can make the attack_carrier run slower because the loss must be calculated at each step.
              The loss is not calculated as part of the normal SPSA optimization procedure.
              For most reasonable choices of hyperparameters, early stopping makes the attack_carrier much faster because
              it decreases the number of steps dramatically.
      :param is_debug: A bool. If True, print debug info for attack_carrier progress.
    ...
    function which you can use to write your own attack_carrier methods. The method uses a tf.while_loop to
    optimize a loss function in a single sess.run() call.
    """
    assert num_steps is not None
    if is_debug:
        with tf.device("/cpu:0"):
            tf.print("Starting PGD attack_carrier with epsilon: %s" % epsilon)
