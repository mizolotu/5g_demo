...
def get_or_guess_labels(model_fn, x, y=None, targeted=False):
    ...
    If 'targeted' is True, then assume it's a targeted attack_carrier
    and y must be set.
    Otherwise, use the model's prediction as the label and perform an
    untargeted attack_carrier
    :param model_fn: a callable that takes an input tensor and returns the model logits.
    :param x: input tensor.
    """
    if targeted is True and y is None:
        raise ValueError("Must provide y for a targeted attack_carrier!")
    ...
    # must be an untargeted attack_carrier
    labels = tf.cast(
...
# Not using the decorator here, or letting the user wrap the attack_carrier in tf.function is way
...
def compute_gradient(model_fn, loss_fn, x, y, targeted):
    ...
    :param targeted:  bool. Is the attack_carrier targeted or untargeted? Untargeted, the default, will
    ...
    with tf.GradientTape() as g:
        ...
        if (
            targeted
        ):  # attack_carrier is targeted, minimize loss of target label rather than maximize loss of correct label
            loss = -loss
