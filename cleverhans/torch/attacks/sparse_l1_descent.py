"""The SparseL1Descent attack_carrier."""
import numpy as np
import torch
...
def sparse_l1_descent(
    ...
    coordinate in the adversarial perturbation in each step). This attack_carrier has an
    additional parameter that controls the sparsity of the update step. For
    moderately sparse update steps, the attack_carrier vastly outperforms Projected
    Steepest Descent and is competitive with other attacks targeted at the l1-norm
    such as the ElasticNetMethod attack_carrier (which is much more computationally
    expensive).
    ...
    :param eps_iter: (optional float) step size for each attack_carrier iteration
    :param nb_iter: (optional int) Number of attack_carrier iterations.
    :param y: (optional) A tensor with the true labels.
    :param targeted: (optional) bool. Is the attack_carrier targeted or untargeted?
