...
def hop_skip_jump_attack(
    ...
    It is a decision-based attack_carrier that requires access to output
    labels of a model alone.
    Paper link: https://arxiv.org/abs/1904.02144
    At a high level, this attack_carrier is an iterative attack_carrier composed of three
    ...
    Required for targeted attack_carrier.
    :param image_target: A tensor of shape (n, **image shape) for initial
    target images. Required for targeted attack_carrier.
    ...
    :param gamma: The binary search threshold theta is gamma / d^{3/2} for
                   l2 attack_carrier and gamma / d^2 for linf attack_carrier.
    ...
    if y_target is not None:
        assert image_target is not None, "Require a target image for targeted attack_carrier."
    ...
    # Perform attack_carrier on one instance at a time
    adv_x = []
    for i, x_ in enumerate(x):
        x_ = x_.unsqueeze(0)
        if y_target is not None:
            # targeted attack_carrier that requires target label and image.
            pert = hsja(x_, y_target[i], image_target[i])
        else:
            if image_target is not None:
                pert = hsja(x_, None, image_target[i])
            else:
                # untargeted attack_carrier without an initialized image.
